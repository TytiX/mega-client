...
class MutableFile extends file_1.File {
        ...
        if (opt.k) {
            ...
            for (let idKeyPair of idKeyPairs) {
                ...
                if (shareKey) {
                    ...
                    aes = KEY_CACHE[id];
                    if (!aes) {
                        aes = KEY_CACHE[id] = new crypto_1.AES(shareKey);
                    }
                    break;
                }
            }
    ...
    upload(opt, source, cb) {
        ...
        if (!opt.target)
            opt.target = this;
        let finalKey;
        let key = crypto_1.formatKey(opt.key);
        if (!key)
            key = secure_random_1.default(24);
        ...
        if (opt.uploadCiphertext) {
            finalKey = key;
            key = crypto_1.unmergeKeyMac(key).slice(0, 24);
        }
        opt.key = key;
        const hashes = [];
        const checkCallbacks = (err, type, hash, encrypter) => {
            ...
            if (!hashes[0])
                return;
            const at = MutableFile.packAttributes(opt.attributes);
            crypto_1.getCipher(finalKey).encryptCBC(at);
            const storedKey = Buffer.from(finalKey);
            this.storage.aes.encryptECB(storedKey);
            const fileObject = {
                h: crypto_1.e64(hashes[0]),
                t: 0,
                a: crypto_1.e64(at),
                k: crypto_1.e64(storedKey)
            };
            if (hashes.length !== 1) {
                fileObject.fa = hashes.slice(1).map((hash, index) => {
                    return index + '*' + crypto_1.e64(hash);
                }).filter((e) => e).join('/');
            }
            const request = {
                a: 'p',
    ...
    _upload(opt, source, type, cb) {
        const encrypter = opt.uploadCiphertext
            ? through_1.default()
            : crypto_1.megaEncrypt(opt.key);
        const pause = through_1.default().pause();
        let stream = stream_combiner_1.default(pause, encrypter);
        ...
        if (size) {
            ...
            stream = stream_combiner_1.default(util_1.detectSize((size) => {
                this._uploadWithSize(stream, size, encrypter, pause, type, opt, cb);
            }), stream);
        }
        // handle stream
    ...
    _uploadAttribute(opt, source, type, cb) {
        const gotBuffer = (err, buffer) => {
            ...
            const encrypter = opt.handle
                ? crypto_1.getCipher(opt.key)
                : new crypto_1.AES(opt.key.slice(0, 16));
            encrypter.encryptCBC(buffer);
            const pause = through_1.default().pause();
            let stream = stream_combiner_1.default(pause);
            stream.end(buffer);
        ...
        util_1.streamToCb(source, gotBuffer);
    }
    _uploadWithSize(stream, size, source, pause, type, opt, cb) {
        ...
        const sendChunk = () => {
            ...
            uploadBuffer = null;
            position += chunkSize;
            util_1.streamToCb(httpreq, (err, hash) => {
    ...
    moveTo(target, cb) {
        ...
        if (!(target instanceof file_1.File)) {
    ...
    setAttributes(attributes, cb) {
        ...
        crypto_1.getCipher(this.key).encryptCBC(newAttributes);
        this.api.request({ a: 'a', n: this.nodeId, at: crypto_1.e64(newAttributes) }, () => {
    ...
    setLabel(label, cb) {
        if (typeof label === 'string')
            label = file_1.LABEL_NAMES.indexOf(label);
    ...
    link(options, cb) {
        ...
        this.api.request({ a: 'l', n: this.nodeId }, (err, id) => {
            ...
            if (!options.noKey && this.key)
                url += `#${crypto_1.e64(folderKey || this.key)}`;
            cb(null, url);
        });
        return this;
    }
    shareFolder(options, cb) {
        ...
        let shareKey = crypto_1.formatKey(options.key);
        if (!shareKey) {
            shareKey = secure_random_1.default(16);
        }
        ...
        const request = {
            ...
            n: handler,
            s: [{ u: 'EXP', r: 0 }],
            ok: crypto_1.e64(this.storage.aes.encryptECB(Buffer.from(shareKey))),
            ha: crypto_1.e64(authKey),
    ...
    importFile(sharedFile, cb) {
        ...
        if (typeof sharedFile === 'string')
            sharedFile = file_1.File.fromURL(sharedFile);
        if (!(sharedFile instanceof file_1.File))
        ...
        const afterGotAttributes = (err, file) => {
            ...
            crypto_1.getCipher(file.key).encryptCBC(attributes);
            ...
            const request = {
                ...
                n: [{
                        ph: downloadId,
                        t: 0,
                        a: crypto_1.e64(attributes),
                        k: crypto_1.e64(this.storage.aes.encryptECB(file.key))
                    }]
            };
            this.api.request(request, (err, response) => {
                if (err)
...
exports.default = MutableFile;
//# sourceMappingURL=mutable-file.js.map