...
const mutable_file_1 = tslib_1.__importDefault(require("./mutable-file"));
class Storage extends events_1.EventEmitter {
    constructor(options, cb) {
        ...
        this.api = new api_1.API(options.keepalive);
        this.files = {};
    ...
    login(cb) {
        ...
        const handleV1Account = (cb) => {
            const pw = crypto_1.prepareKey(Buffer.from(this.options.password));
            // after generating the AES key the password isn't needed anymore
            delete this.options.password;
            const aes = new crypto_1.AES(pw);
            const uh = crypto_1.e64(aes.stringhash(Buffer.from(this.email)));
            const request = { a: 'us', user: this.email, uh };
            finishLogin(request, aes, cb);
        };
        const handleV2Account = (info, cb) => {
            crypto_1.prepareKeyV2(Buffer.from(this.options.password), info, (err, result) => {
                if (err)
                    return cb(err);
                // after generating the AES key the password isn't needed anymore
                delete this.options.password;
                const aes = new crypto_1.AES(result.slice(0, 16));
                const uh = crypto_1.e64(result.slice(16));
                const request = { a: 'us', user: this.email, uh };
                finishLogin(request, aes, cb);
            });
        };
        const finishLogin = (request, aes, cb) => {
            this.api.request(request, (err, response) => {
                if (err)
                    return cb(err);
                this.key = crypto_1.formatKey(response.k);
                aes.decryptECB(this.key);
                this.aes = new crypto_1.AES(this.key);
                const t = crypto_1.formatKey(response.csid);
                const privk = this.aes.decryptECB(crypto_1.formatKey(response.privk));
                const rsaPrivk = rsa_1.cryptoDecodePrivKey(privk);
                if (!rsaPrivk)
                    throw Error('invalid credentials');
                let sid = crypto_1.e64(rsa_1.cryptoRsaDecrypt(t, rsaPrivk).slice(0, 43));
    ...
    reload(force, cb) {
        ...
        this.api.request({ a: 'f', c: 1 }, (err, response) => {
            ...
            this.shareKeys = response.ok.reduce((shares, share) => {
                ...
                if (crypto_1.constantTimeCompare(crypto_1.formatKey(share.ha), auth)) {
                    shares[handler] = this.aes.decryptECB(crypto_1.formatKey(share.k));
                }
    ...
    _importFile(f) {
        ...
        if (!this.files[f.h]) {
            const file = this.files[f.h] = new mutable_file_1.default(f, this);
    ...
    static fromJSON(json) {
        ...
        storage.key = crypto_1.d64(json.key);
        storage.aes = new crypto_1.AES(storage.key);
