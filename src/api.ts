...
export class API extends EventEmitter {
  ...
  request (json: any, cb: any, retryno = 0) {
    const qs: any = {id: (this.counterId++).toString()}
    if (this.sid) {
      qs.sid = this.sid
    }
    ...
    this.requestModule({
      ...
      if (!err && (typeof resp === 'number') && resp < 0) {
        ...
        err = Error(Object.values(ERRORS)[-resp +1])
      } else {
  ...
  pull (sn: any, retryno = 0) {
    this.sn = this.requestModule({
      ...
      if (!err && (typeof resp === 'number') && resp < 0) {
        ...
        err = Error(Object.values(ERRORS)[-resp])
      }
      if (err) throw err
