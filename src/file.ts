...
export class File extends EventEmitter {
  downloadId: any;
  key: string;
  type: number; // 1 is directory, 0 is file
  directory: boolean;
  api: any;
  loadedFile: any;
  ...
  checkConstructorArgument (value: any) {
  ...
  loadAttributes (cb: (err?: Error, file?: File) => void) {
    ...
    this.api.request(req, (err: any, response: any) => {
      if (err) return cb(err)

      if (this.directory) {
        const filesMap = Object.create(null)
        const nodes = response.f
        const folder = nodes.find((node: any) => node.k &&
  ...
  download (options?: any, cb?: any) {
    ...
    cs.request(req, (err: any, response: any) => {
      ...
      function handleMegaErrors (resp: any) {
      ...
      function handleConnectionErrors (err: any) {
      ...
      if (maxConnections === 1) {
        ...
        combined.on('error', (err: Error) => stream.emit('error', err))
  ...
  link (options: any, cb: (err: string | null, url: string) => {}) {
  ...
  static fromURL (opt: string | any) {
    ...
    if (url.path && url.path.match(/\/(file|folder)\//) !== null) {
      ...
      const fileHandler = url.path ? url.path.substring(
  ...
  static unpackAttributes(at: any) {
