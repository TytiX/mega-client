...
import { e64, getCipher, megaEncrypt, formatKey, AES, unmergeKeyMac } from './crypto'
...
class MutableFile extends File {
  ...
  mkdir (opt: any, cb: any) {
    ...
    this.api.request(request, (err: any, response: any) => {
    ...
    function returnError (e: any) {
      if (cb) cb(e)
    }
  }

  upload (opt: any, source: any, cb: any) {
    ...
    const hashes: any = []
    const checkCallbacks = (err: any, type: any, hash: any, encrypter: any) => {
      if (err) return returnError(err)
      ...
      if (hashes.length !== 1) {
        fileObject.fa = hashes.slice(1).map((hash: any, index: any) => {
  ...
  _upload (opt: any, source: any, type: any, cb: any) {
    ...
    if (size) {
      ...
      stream = pipeline(detectSize((size: any) => {
        this._uploadWithSize(stream, size, encrypter, pause, type, opt, cb)
      }), stream)
    }
  ...
  _uploadAttribute (opt: any, source: any, type: any, cb: any) {
    const gotBuffer = (err: any, buffer: any) => {
  ...
  _uploadWithSize (stream: any, size: any, source: any, pause: any, type: any, opt: any, cb: any) {
    const ssl = (process.env.IS_BROWSER_BUILD || opt.forceHttps) ? 2 : 0
    const getUrlRequest: any = type === 0
    ...
    let uploadBuffer: any, uploadURL: any
    let chunkSize: any, chunkPos: any
    ...
    const sendChunk = () => {
      ...
      httpreq.on('response', (response: any) => {
      ...
      streamToCb(httpreq, (err: any, hash: any) => {
    ...
    this.api.request(getUrlRequest, (err: any, resp: any) => {
      if (err) return cb(err)
      uploadURL = resp.p
      handleChunk()
    })
  ...
  uploadAttribute (type: any, data: any, callback: any) {
    ...
    this._uploadAttribute({
      key: this.key,
      handle: this.nodeId
    }, data, type + 1, (err: any, streamType: any, hash: any, encrypter: any) => {
  ...
  moveTo (target: any, cb: any) {
    ...
    const request: any = {a: 'm', n: this.nodeId, t: target.nodeId}
  ...
  setAttributes (attributes: any, cb: any) {
  ...
  setFavorite (isFavorite: any, cb: any) {
    this.setAttributes({
      fav: isFavorite ? 1 : 0
    }, cb)
  ...
  link (options: any, cb: any) {
    ...
    this.api.request({a: 'l', n: this.nodeId}, (err: any, id: any) => {
      if (err) return cb(err)
  ...
  shareFolder (options: any, cb: any) {
    ...
    this.api.request(request, (err: any) => {
      if (err) return cb(err)
      this.link(Object.assign({
  ...
  unshareFolder (options: any, cb: any) {
    const request = {
      a: 's2',
      n: this.nodeId,
      s: [{u: 'EXP', r: ''}]
  ...
  importFile (sharedFile: any, cb: any) {
    ...
    const afterGotAttributes = (err: any, file: any) => {
      if (err) return cb(err)
  ...
  static packAttributes (attributes: any) {
    let at: any = JSON.stringify(attributes)
...
function makeCryptoRequest (storage: any, sources: any, shares?: any) {
  ...
  if (!shares) {
    shares = sources
      .map((source: any) => getShares(shareKeys, source))
      .reduce((arr: any, el: any) => arr.concat(el))
      .filter((el: any, index: any, arr: any) => index === arr.indexOf(el))
  }

  const cryptoRequest = [
    shares,
    sources.map((node: any) => node.nodeId),
    []
  ]
...
function selfAndChildren (node: any) {
  return [node]
...
function getShares (shareKeys: any, node: any): any {
