...
class Storage extends EventEmitter {
  ...
  login (cb: any) {
    ...
    const loadUser = (cb: any) => {
      this.api.request({a: 'ug'}, (err: any, response: any) => {
        if (err) return cb(err)
        this.name = response.name
        this.user = response.u

        if (this.options.autoload) {
          this.reload(true, (err: any) => {
            if (err) return cb(err)
            ready()
          })
        } else {
    ...
    const handleV2Account = (info: any, cb: any) => {
      prepareKeyV2(Buffer.from(this.options.password), info, (err: any, result: any) => {
        if (err) return cb(err)
    ...
    const finishLogin = (request: any, aes: any, cb: any) => {
    ...
    this.api.request({a: 'us0', user: this.email}, (err: any, response: any) => {
      if (err) return cb(err)
  ...
  reload (force: any, cb: any): any {
    ...
    this.mounts = []
    this.api.request({a: 'f', c: 1}, (err: any, response: any) => {
      if (err) return cb(err)

      this.shareKeys = response.ok.reduce((shares: any, share: any) => {
      ...
      response.f.forEach((file: any) => this._importFile(file))
      cb(null, this.mounts)
    })
  ...
  upload (opt: any, buffer: any, cb: any) {
    // Wait for ready event.
    if (this.status !== 'ready') {
      this.on('ready', () => {
  ...
  getAccountInfo (cb: any) {
    this.api.request({a: 'uq', strg: 1, xfer: 1, pro: 1}, (err: any, response: any) => {
      if (err) cb(err)
      const account: any = {}
